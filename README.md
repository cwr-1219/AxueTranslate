...
* **🔍 划词弹窗免替换 (新增)**：选中文本后按下弹窗热键（默认 `Ctrl + Alt + F`），将在鼠标偏右下方弹出一个高质感毛玻璃半透明浮窗展示翻译结果。支持一键快速复制、支持在浮窗内直接切换翻译目标语言并重新异步翻译（且两端配置双向联动同步）、支持滚动条极简圆角美化、失焦（点击外部）或按 `Esc` 键自动淡出销毁，纯粹浏览不污染原输入框。
...
### 1. 下载与安装
我们在 GitHub 的 [Releases 页面](https://github.com/cwr-1219/AxueTranslate/releases/tag/v1.0.1) 提供了两种分发格式：
* **标准安装包 (`AxueTranslate_Setup.exe`)**：运行即可一键安装并强制覆盖旧版本，自动生成桌面及开始菜单快捷方式。
* **免安装绿色版 (`AxueTranslate_v1.0.1_windows_x64.zip`)**：解压即可直接运行主程序 `AxueTranslate.exe`。
